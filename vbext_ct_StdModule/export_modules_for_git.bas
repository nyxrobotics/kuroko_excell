...
Sub ExportAll()
    ...
    For Each module In moduleList
        ...
        If (module.Type = vbext_ct_ClassModule) Then
            extension = "cls"
            sSubdir = "vbext_ct_ClassModule"
        '// フォーム
        ElseIf (module.Type = vbext_ct_MSForm) Then
            '// .frxも一緒にエクスポートされる
            extension = "frm"
            sSubdir = "vbext_ct_MSForm"
        '// 標準モジュール
        ElseIf (module.Type = vbext_ct_StdModule) Then
            extension = "bas"
            sSubdir = "vbext_ct_StdModule"
        '//     Excelオブジェクト
        ElseIf (module.Type = vbext_ct_Document) And (module.Name = "ThisWorkbook") Then
            extension = "bas"
            sSubdir = "vbext_ct_Document"
        '// その他
        Else
            '// エクスポート対象外のため次ループへ
            GoTo CONTINUE
        ...
        '// エクスポート実施
        If sSubdir <> "" Then
            If Dir(sPath & "\" & sSubdir, vbDirectory) = "" Then
                MkDir (sPath & "\" & sSubdir)
            End If
        End If
        
        sFilePath = sPath & "\" & sSubdir & "\" & module.Name & "." & extension
